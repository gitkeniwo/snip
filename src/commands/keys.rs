...
fn binding_source(defaults: &Keymap, mode: Mode, chord: Chord, id: CommandId) -> BindingSource {
    if binding_in_mode(defaults, mode, chord) == Some(id) {
...
fn binding_in_mode(keymap: &Keymap, mode: Mode, chord: Chord) -> Option<CommandId> {
    keymap
        .resolve_with_mode(&[mode], chord)
        .filter(|(resolved_mode, _)| *resolved_mode == mode)
        .map(|(_, id)| id)
}

const EXPORT_HEADER: &str = "# Key bindings for snip. Generated by `snip keys export`; edit as needed.\n\
# Values are a chord string or a list of chord strings; use [] to unbind an action.\n\
# With `inherit-defaults = false`, this file is authoritative. Without it, listed\n\
# actions replace their built-in defaults and all other defaults remain available.\n";

fn export_toml(modes: &[Mode], authoritative: bool) -> String {
    let defaults = Keymap::defaults();
    let mut output = EXPORT_HEADER.to_owned();
    if authoritative {
...
fn cross_mode_shadowing(keymap: &Keymap, defaults: &Keymap) -> Vec<CheckDiagnostic> {
    ...
    for (mode, inherited, allowlist_only) in relationships {
        for (chord, action) in keymap.bindings_for(mode) {
            let Some(inherited_action) = binding_in_mode(keymap, inherited, chord) else {
                continue;
            };
...
mod tests {
    ...
    fn export_is_authoritative_stable_and_mode_filtered() {
        ...
        assert!(full_export.starts_with(EXPORT_HEADER));
        assert!(full_export.contains("inherit-defaults = false\n\n[global]\n"));

        let export = export_toml(&[Mode::Global], false);
        assert!(export.starts_with(EXPORT_HEADER));
        assert!(
            !export
                .lines()
